...
void Player::moveOrAttack(int dir)
{
	...
	case UP:
		if (m_row == 1)
			return;
		else if (m_arena->numZombiesAt(m_row - 1, m_col))
	...
	case LEFT:
		if (m_col == 1)
			return;
		else if (m_arena->numZombiesAt(m_row, m_col - 1))
...
void Arena::display() const
{
	...
	for (int i = 0; i < m_nZombies; i++)
	{
		...
		{
			zombieRow = m_zombies[i]->row() - 1;
			zombieCol = m_zombies[i]->col() - 1;
			char& zombieChar = grid[zombieRow][zombieCol];
			switch (zombieChar)
			{
			case '.':
				zombieChar = 'Z';
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
	if (numZombiesAt(r, c))
	{
		...
		if (m_zombies[zombieIndex]->getAttacked(dir))
		{
			m_nZombies--;
			delete m_zombies[zombieIndex];
			m_zombies[zombieIndex] = m_zombies[m_nZombies];
			return true;
		}
		else
			return false;
